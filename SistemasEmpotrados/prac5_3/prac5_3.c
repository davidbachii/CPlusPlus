...
#define print_time(_s1, _tb, _s2) \
  do { \
    printf( "%s%02d:%02d:%02d   %02d/%02d/%04d%s", \
       ...
       (_tb)->day, (_tb)->month, (_tb)->year, _s2 ); \
    fflush(stdout); \
  } while ( 0 )
...
rtems_task Init(rtems_task_argument argument){
  ...
  time.minute = 0;
  time.second = 0;
  time.ticks  = get_ticks_since_boot();
...
rtems_task TAvoidObstacles(rtems_task_argument unused){
	...
	do{
		...
		rtems_semaphore_obtain(MutexName, RTEMS_DEFAULT_OPTIONS, RTEMS_NO_TIMEOUT);

	    puts("Do Avoid Obstacles");
